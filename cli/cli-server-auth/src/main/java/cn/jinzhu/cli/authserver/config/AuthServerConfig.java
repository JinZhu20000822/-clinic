package cn.jinzhu.cli.authserver.config;
...
public class AuthServerConfig extends AuthorizationServerConfigurerAdapter { //进行认证配置
    ...
    @Autowired
    public DataSource dataSource;/*认证信息从数据库获取*/

    @Bean   /*获取数据库里面的token令牌，数据库有一个认证信息表，不保存到数据库可以保存到内存只是重新启动就没了*/
...
//    }
    @Bean/*从数据库客户端详情信息表中查询 允许使用oauth账号登录第三方项目信息*/
